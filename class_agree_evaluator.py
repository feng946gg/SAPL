...
'''
python class_agree_evaluator.py 
    --name="TTYD_stop_agree_synth_sk" 
    --resume_path='ckpts_bn/TTYD_core_synth_sk'
...
def validation_consistency(net, net_later, config, test_loader, epoch, disable_log, device, criterion_model_train=True):
    ...
    #将网络设置为评估模式
    net.eval()
    #如果criterion_model_train为True，则将net_later设置为训练模式，否则设置为评估模式
