...
class Pandaset(Dataset):
    def __init__(self, root, split="training", transform=None, dataset_size=None, multiframe_range=None,
        ...
        for seq in sequences:
            ...
            all_files_training += [x for x in Path(os.path.join(root, seq, "lidar")).rglob('*.pkl') if
                                   int(str(x).split("/")[-1].split(".")[0]) < 50]  # The first 50 frames of a sequence
            all_labels_training += [x for x in Path(os.path.join(root, seq, "annotations", "semseg")).rglob('*.pkl') if
                                    int(str(x).split("/")[-1].split(".")[0]) < 50]  # The first 50 frames of a sequence
            all_poses_training += _poses[0:50]

            all_files_test += [x for x in Path(os.path.join(root, seq, "lidar")).rglob('*.pkl') if
                               int(str(x).split("/")[-1].split(".")[0]) >= 50]  # The last 30 frames of a sequence
            all_labels_test += [x for x in Path(os.path.join(root, seq, "annotations", "semseg")).rglob('*.pkl') if
                                int(str(x).split("/")[-1].split(".")[0]) >= 50]  # The first 50 frames of a sequence
        ...
        print(len(all_poses_training))  # 1500
        print(len(all_files_training))
        print(len(all_labels_training))
        # assert len(all_poses_training) == len(all_files_training) == len(all_labels_training) == 3800
        assert len(all_poses_training) == len(all_files_training) == len(all_labels_training)
        print(len(all_files_test))  # 900
        print(len(all_labels_test))
        # assert len(all_poses_test) == len(all_files_test) == len(all_labels_test) == 2280
        assert len(all_poses_test) == len(all_files_test) == len(all_labels_test)
